...
轉換功能提供三種類型的導出器，用於將[!DNL .vsl]檔案、日誌檔案、XML檔案和ODBC資料導出為[!DNL .vsl]檔案、文本檔案或分隔文本檔案，這些文本檔案可用於DataWarehouse載入常式、審計機構或其他目標。
...
1. 在資料工作台中開啟[!DNL Transform.cfg]。 請參閱[若要編輯Insight Transform.cfg檔案](../../../../../home/c-dataset-const-proc/c-transf-func/c-config-files-transf/t-ins-transf-file/t-ins-transf-file.md#task-857fc535ccdb4c39b763179efa4b0f13)。
...
1. 選擇下列選項之一：
   ...
   * **[!UICONTROL ExportVSLFile]**
   ...
   >[!NOTE]
   >
   >對於[!DNL ExportVSLFile]選項，輸入檔案中的所有擴展欄位和所有以cs(*header*)形式自定義的欄位始終寫入輸出VSL檔案。 如果覆寫現有的擴充欄位，新值會寫入輸出檔案，即使欄位空白亦然。
...
1. 使用下表作為指南編輯配置檔案中的Exportors參數：
   ...
   <table id="table_35C380DB6E4F42448C149D5EC185213C"> 
    ...
    <tbody> 
      ...
      <td colname="col2"> <p>僅適用於<span class="wintitle"> ExportTextFile</span>。 每個輸出行的格式，由欄位名轉義（表示為%<i>欄位名</i>%）和任何其他需要的固定文本組成。 格式應包含行分隔符，通常為[CR] [LF]。 </p> <p> 常值百分比符號(%)可透過逸出字元嵌入格式字串，如下所示：%% </p> <p> 「資料格式」參數的項目範例為<span class="filepath"> %x-timestring% %x-trackingid%[CR][LF]</span>。 </p> </td> 
      </tr> 
      <tr> 
      <td colname="col1"> 欄位 </td> 
      <td colname="col2">僅適用於<span class="wintitle"> ExportDelimitedTextFile</span>。 要輸出的欄位的名稱。 </td> 
      </tr> 
      <tr> 
      <td colname="col1"> 分隔字元 </td> 
      <td colname="col2"> <p>選填。僅適用於<span class="wintitle"> ExportDelimitedTextFile</span>。 用於分隔輸出檔案中欄位的字元。 </p> <p> 軟體無法轉義包含在資料值中的分隔字元。 因此，Adobe不建議使用逗號做為分隔字元。 </p> <p> 如果您按住Ctrl鍵並在分隔字元參數內按一下滑鼠右鍵，則會出現<span class="wintitle">插入</span>功能表。 此功能表包含常用作分隔字元的特殊字元清單。 </p> </td> 
      </tr> 
      <tr> 
      <td colname="col1"> 行分隔符 </td> 
      <td colname="col2">選填。僅適用於<span class="wintitle"> ExportDelimitedTextFile</span>。 用於在輸出檔案中分隔行的字元。 預設值為[CR] [LF]。 </td> 
      </tr> 
      <tr> 
      <td colname="col1"> 名稱 </td> 
      <td colname="col2"> <p>選填。導出器的標識符。 此名稱顯示在<span class="wintitle"> Detailed Status</span>介面中。 </p> <p> 有關<span class="wintitle">詳細狀態</span>介面的資訊，請參閱<i>Data Workbench使用手冊</i>。 </p> </td> 
      </tr> 
      <tr> 
      ...
      <td colname="col2"> <p>儲存輸出檔案的路徑。 路徑是相對於資料工作台伺服器安裝資料夾的。 </p> <p> <p>注意：儲存輸出資料的資料工作台伺服器是處理伺服器#0，位於<span class="filepath"> profile.cfg</span>檔案中。 </p> </p> </td> 
      </tr> 
      <tr> 
      ...
      <ul id="ul_64F56D093E2E4D07ACB7D7921F4E6FA1"> 
       ...
       <li id="li_EE8CF71BA12149F49D4B7F7108262CD0"> NONE. 不執行任何旋轉。 所有資料都會寫入相同的檔案（或由其他參數設定所決定的一組檔案）。 請參閱本表中的<span class="wintitle">檔案名格式</span>參數。 </li> 
      </ul> <p>預設的檔案旋轉期間為DAY。 </p> 
      <ul id="ul_0F3BC98275634F759E5022FF2C19715E"> 
       <li id="li_24DC4D144DA94ED0B7B50E8BB39DB8E3"> 僅在<span class="wintitle">離線模式</span>中工作時，才將檔案旋轉設定為NONE。 請參閱<a href="../../../../../home/c-dataset-const-proc/c-transf-func/c-config-files-transf/t-ins-transf-file/t-ins-transf-file.md#task-857fc535ccdb4c39b763179efa4b0f13">離線模式</a>參數說明。 </li> 
      </ul> </td> 
      </tr> 
      <tr> 
      <td colname="col1"> 檔案名格式 </td> 
      <td colname="col2"> <p>選填。輸出檔案名的格式。 </p> <p> 每個日誌條目都可以儲存在一個檔案中，其名稱是從旋轉週期的開始時間派生的，也可以從包含的行中的欄位值導出的。 檔案名中使用的欄位被嵌入為欄位名轉義（表示為%<i>欄位名</i>%）。 </p> <p>與旋轉週期相關的檔案名元件使用下列轉義序列嵌入格式字串中： 
      ...
      <ul id="ul_07AE3624E7D74632AD5E5F164048196F"> 
       ...
       <li id="li_C64731961ED6402FB92210A42854BA72"> 如果<span class="wintitle">檔案名格式</span>未導致每個旋轉週期的檔案名唯一，則會生成錯誤（請參閱本表中的檔案旋轉週期參數）。 例如，使用DAY旋轉期間時，%dd%、%mm%和%yy%或%yyy%轉義序列必須出現在模式中，以避免資料遺失。 </li> 
...
1. 在[!DNL Transform.cfg]檔案中定義匯出器（並變更其他參數）後，請將檔案儲存在本機，並儲存至資料工作台伺服器機器上的適當描述檔。
