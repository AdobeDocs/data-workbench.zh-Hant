...
<table id="table_05B1FBB2AED242D99081E62BE2FBEC60"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2">True 或 False. 僅當類型為<span class="wintitle"> LEXMARID</span>時使用。 若設為false，大寫和小寫字母會視為相等。 </td> 
  ...
  <tr> 
   ...
   <td colname="col2">要進行的比較類型。 可用類型為<span class="wintitle"> LEXMARID</span>、<span class="wintitle"> NUMERIC</span>和<span class="wintitle"> DATETIME</span>。 有關類型的說明，請參閱<a href="../../../../home/c-dataset-const-proc/c-conditions/c-test-ops/c-test-types.md#concept-a9fca97a2f03464cb0cbab8b5f809d0a">測試操作的測試類型</a>。 </td> 
   <td colname="col3"> <span class="wintitle"> 辭匯</span> </td> 
  </tr> 
 </tbody> 
</table>

此範例使用[!DNL Compare]條件來定義[!DNL Log Entry Condition]。 Data Workbench伺服器讀取每個事件資料記錄時，會比較數值x-age與55。 若指定的記錄項目x年齡小於或等於55，記錄項目會納入資料集建置程式中。

![](assets/cfg_Condition_CompareCondition.png)

## 非空{#section-1decb9d887894073a1b6b3d985729ac8}

[!DNL Not Empty]條件會檢查欄位，查看其是否包含值或空白。 對於[!DNL Input]欄位值不為空的任何日誌條目，都滿足該條件。
...
[!DNL Range]條件取用輸入欄位，並確定該欄位的值是否落在給定的最小(Min)和最大(Max)參數值內，包括。
...
<table id="table_1587D8D333804FC28024C0DFC2F2D4D3"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> 區分大小寫 </td> 
   <td colname="col2">True 或 False. 僅當<span class="wintitle">類型</span>為<span class="wintitle"> EXCLIADARY</span>時使用。 若設為false，大寫和小寫字母會視為相等。 </td> 
   <td colname="col3"> true </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 註解 </td> 
...
此範例使用[!DNL Range]條件來定義[!DNL Log Entry Condition]。 當Data Workbench伺服器讀取每個[!DNL event data]記錄時，會比較數值x-age與55。 若對於指定的記錄項目，x-age至少為55，則記錄項目會包含在資料集建構程式中。 此示例執行與[!DNL Compare]條件示例相同的函式。 請參閱[Compare](../../../../home/c-dataset-const-proc/c-conditions/c-test-ops/c-test-op-con.md#section-fb2bdb3838504099b324b9838cdeeaac)。
...
[!DNL Regular Expression]條件測試使用規則運算式模式比對（請參閱[規則運算式](../../../../home/c-dataset-const-proc/c-reg-exp.md#concept-070077baa419475094ef0469e92c5b9c)）來判斷指定輸入欄位的值是否包含符合「比對」參數中指定其中一個模式的字串。

如果輸入是字串的向量，則測試僅使用向量中的第一個值。 [!DNL Regular Expression]條件會執行完整字串比較。 若要識別子字串，您必須在開頭附加「」。*」到字串。
...
<table id="table_0BF5F89F87C9493B8DABA97620074FAD"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> 區分大小寫 </td> 
   <td colname="col2"> True 或 False. 若設為false，大寫和小寫字母會視為相等。 </td> 
   <td colname="col3"> true </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 註解 </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>與輸入欄位值相符的規則運算式模式。 </p> <p> <b> 若要新增規則運算式模式</b> 
     <ol id="ol_6D6467FF74334DEA8E8625C3B155D11D"> 
      <li id="li_9E13A63558FF44749C2E49BD50B7F770">按一下右鍵<span class="uicontrol">「匹配</span>」。 </li> 
...
此範例說明如何使用[!DNL Regular Expression]條件來比對從網站流量收集的資料欄位。 只有在cs(referrer-query)欄位包含符合規則運算式`campaign=C[1-9][0-9]{4}`的字串時，條件才會傳回true。 此規則運算式與包含`campaign=C12345`的任何字串相符。 但該模式與字串`campaign=C0123&`不匹配，因為`C`之後的第一個字元不在`1-9`範圍內。

![](assets/cfg_Condition_RegularExpression.png)

## 字串符合{#section-f8d132085c6b4500bfbe4515b848142f}

[!DNL String Match]條件會測試字串相等性。 它會將指定欄位作為輸入，並根據操作的Matches參數中指定的字串，測試每個記錄項目中該欄位的值。 如果這些區分大小寫的相符字串中的任何一個與所提供的輸入欄位中的值相同，則操作會傳回true。 若[!DNL StringCondition]不包含相符字串，則條件會傳回false。 如果輸入是字串的向量，則測試僅使用向量中的第一個值（字串）。
  ...
  <tr> 
   ...
   <td colname="col2"> <p>與輸入欄位值相符的字串。 </p> <p> <b>新增字串的方式</b> 
     <ol id="ol_9E32218C771445D88357960475FAD6EB"> 
      <li id="li_A700747858D0470491783E9B3933DAFE">按一下右鍵<span class="uicontrol">「匹配</span>」。 </li> 
...
此範例使用從網站流量收集的資料來說明[!DNL String Match]條件的使用。 條件會測試輸入欄位(cs-uri-stem)是否符合「符合」參數中指定的兩個字串之一，如果欄位cs-uri-stem是確切字串[!DNL /navigation/footer.asp]或確切字串[!DNL /navigation/header.asp]，則成功。
