...
<table id="table_5FF491A7A6C24E43A06A5C344BF05430"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>僅在使用Adobe咨詢服務時設定此參數。 </p> <p>啟用Web模組和發射器的調試記錄。 </p> <p>當<span class="wintitle">感測器</span>無法正常工作時，可使用此參數。 設定此參數後，您必須在指定的路徑位置建立空白檔案，並為所有使用者提供「寫入」權限。 例如（在Web伺服器上的unix殼層內）: 
     ...
     </ul> </p> <p>您應該只在短時間內啟用調試日誌記錄，之後應將日誌檔案發送到要分析的Adobe咨詢服務。 </p> <p>範例：<span class="filepath"> DebugLogPath /var/log/vslog.txt</span> </p> <p>Adobe建議先在測試環境中設定此參數，以判斷對系統的影響。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>如果要啟用受控實驗將整個站點或站點的整個子目錄重新映射到另一個位置，請將此參數設定為「on」。 預設值為「off」。 </p> <p>範例：<span class="filepath"> ExpPartialMatch off</span> </p> <p> <p>注意： 將此參數設為「on」時請務必小心。 </p> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>發送器等待發送下一批資料包的秒數。 </p> <p>預設值為 15。 </p> <p>範例：<span class="filepath"> MaxPageLoadTime 15</span> </p> <p> <p>注意： 請勿先聯絡Adobe諮詢服務，變更此參數值。 </p> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 隱私權ID </td> 
   <td colname="col2"> <p>可讓您停用訪客追蹤，以便用來符合退出原則。 </p> <p>啟用後，<span class="wintitle"> Sensor</span>不會記錄任何訪客的「封包」，其V1st Cookie設定為指定的PrivacyID。 由於沒有記錄這些訪客的資訊，因此這些訪客的相關資訊不會傳送至<span class="keyword">資料工作台伺服器</span>以進行處理。 </p> <p>若要啟用此功能，您必須完成下列步驟： 
     <ol id="ol_5D658C5E4AB14F419029E0FFC52F1310"> 
      <li id="li_BF056439F92148169BF592731264C3CD"> 在<span class="wintitle">感測器</span>的<span class="filepath"> txlogd.conf</span>檔案中，PrivacyID必須定義為0（零）。 <p>範例：<span class="filepath"> PrivacyID 0</span> </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>發射器(txlogd)定期傳送請求的位置，以查看網站是否正常運作。 </p> <p>請注意，位置是以下列格式指定，而非URL: </p> <p>http,<i>serverAddress,port,/resource</i> </p> <p>其中<i>serverAddress</i>是伺服器名稱或IP位址，<i>port</i>是伺服器的HTTP監聽埠，而<i>resource</i>是要求的特定資源（可包含查詢字串）。 </p> <p>您可以指定多個SiteTest行。 </p> <p>範例：<span class="filepath"> SiteTest http,localhost,80,/test.html</span> </p> <p> <p>注意： 目前僅支援http。 </p> </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_598C3CDA5AA440228AF88C3BE4A8F77C"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>僅在使用Adobe咨詢服務時設定此參數。 </p> <p>告訴IIS <span class="wintitle"> Sensor</span> ，應該使用哪兩個可能的日誌記錄"hook"來記錄資料包 </p> <p>當IIS <span class="wintitle">感測器</span>未正確記錄資料包時，請使用此參數。 如果預設記錄掛接無法正常運作，此參數會設為「off」。 預設值為「開啟」。 </p> <p>範例：<span class="filepath">在</span>上發送的ISCaptureBytesSent </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> IISUseAlternateHandler </td> 
   <td colname="col2"> <p>僅在使用Adobe咨詢服務時設定此參數。 </p> <p>告訴<span class="wintitle"> Sensor</span>應使用哪兩個可能的"hook"來設定v1st Cookie。 </p> <p>當IIS <span class="wintitle"> Sensor</span>未正確設定v1st Cookie時，可使用此參數。 如果預設的掛接未正確設定v1st Cookie，此參數會設為"yes"。 預設值為"no"。 </p> <p>範例：<span class="filepath"> IISUseAlternateHandler no</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>NewUserCacheControl </p> <p>CacheControl </p> </td> 
   <td colname="col2"> <p>預設情況下，<span class="wintitle">感測器</span>會在每個請求上發送快取控制響應標頭。 啟用快取控制功能時，<span class="wintitle">感測器</span>會向瀏覽器發送一個「過期」報頭，其值為Thu, 1994年12月1日16:00:00 GMT。 </p> <p>您可以視需要在<span class="filepath"> txlogd.conf</span>檔案中編輯下列兩行來修改回應字串： </p> <p> <span class="filepath"> </span> <i>&lt;&gt; NewUserCacheControlstring1</span>&gt;</i><span class="filepath"> </span></p> <p> <span class="filepath"> </span> <i>&lt;&gt; CacheControlstring2</span>&gt;</i><span class="filepath"> </span></p> <p>範例： </p> <p> <span class="filepath"> NewUserCacheControl no-cache=Set-Cookie</span> </p> <p> <span class="filepath"> CacheControl專用，max-age=0,must-revalidate</span> </p> <p>若要停用快取控制回應標頭的傳送，請為每一行輸入連字型大小，如下所示： </p> <p> <span class="filepath"> NewUserCacheControl -</span> </p> <p> <span class="filepath"> CacheControl -</span> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_88696CCA93874BB683538BD614E07890"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>僅在使用Adobe咨詢服務時設定此參數。 </p> <p>告訴<span class="wintitle"> Sensor</span>應使用哪兩個可能的"hook"來設定v1st Cookie。 </p> <p>當Apache <span class="wintitle"> Sensor</span>未正確設定v1st Cookie時，請使用此參數。 如果預設的掛接未正確設定v1st Cookie，此參數會設為"yes"。 預設值為"no"。 </p> <p>範例：<span class="filepath"> ApacheUseAlternateHandler no</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> ApacheUseBothHandlers </td> 
   <td colname="col2"> <p>僅在使用Adobe咨詢服務時設定此參數。 </p> <p>指示<span class="wintitle">感測器</span>嘗試在兩個掛接中設定v1st Cookie。 </p> <p>當Apache <span class="wintitle"> Sensor</span>未正確設定v1st Cookie時，請使用此參數。 預設值為"yes"。 </p> <p>如果設為"yes"，而v1st Cookie未在第一個掛接中正確設定，則會使用第二個掛接。 如果設為"no"，您會設定ApacheUseAlternateHandler參數，以指出要用來設定v1st Cookie的掛接。 </p> <p>範例：<span class="filepath"> ApacheUseBothHandlers yes</span> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>NewUserCacheControl </p> <p>CacheControl </p> </td> 
   <td colname="col2"> <p>預設情況下，<span class="wintitle">感測器</span>會在每個請求上發送快取控制響應標頭。 啟用快取控制功能時，<span class="wintitle">感測器</span>會向瀏覽器發送一個「過期」報頭，其值為Thu, 1994年12月1日16:00:00 GMT。 </p> <p>您可以視需要在<span class="filepath"> txlogd.conf</span>檔案中編輯下列兩行來修改回應字串： </p> <p> <span class="filepath"> </span> <i>&lt;&gt; NewUserCacheControlstring1</span>&gt;</i><span class="filepath"> </span></p> <p> <span class="filepath"> </span> <i>&lt;&gt; CacheControlstring2</span>&gt;</i><span class="filepath"> </span></p> <p>範例： </p> <p> <span class="filepath"> NewUserCacheControl no-cache=Set-Cookie</span> </p> <p> <span class="filepath"> CacheControl專用，max-age=0,must-revalidate</span> </p> <p>若要停用快取控制回應標頭的傳送，請為每一行輸入連字型大小，如下所示： </p> <p> <span class="filepath"> NewUserCacheControl -</span> </p> <p> <span class="filepath"> CacheControl -</span> </p> <p> <p>注意： Adobe建議您不要停用此功能。 </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
