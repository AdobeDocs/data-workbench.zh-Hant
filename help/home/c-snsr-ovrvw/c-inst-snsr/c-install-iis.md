---
description: 有關如何安裝和配置在Microsoft Windows Server 2000或更高版本下運行的Sensor for Internet Information Services(IIS)5.x或6.x的說明。
...
有關如何安裝和配置在Microsoft Windows Server 2000或更高版本下運行的Sensor for Internet Information Services(IIS)5.x或6.x的說明。

使用IIS 6.x時，必須啟用記錄，感測器才能正常工作。 如果您禁用了日誌記錄以減少磁碟I/O，則可以啟用日誌記錄，而無需將任何資料寫入日誌。 若要這麼做，請啟用記錄，然後清除W3C擴充記錄檔格式屬性(Properties for the W3C Extended Log File Format)的「進階」標籤上的所有欄位。 如果您需要協助，請聯絡Adobe諮詢服務。
...
## 2.編輯配置檔案 {#section-206daf855e664f16af9a96a5cd3e62b1}
...
1. 在文字 `<SensorDirectory>/txlogd.conf` 編輯器中開啟檔案，並設定所需參數以及任何所需的選用參數。
   ...
   有關txlogd.conf參數的說明，請參見Sensor Txlogd.conf File Parameters。

   有關已完成配置檔案的示例，請參見Sensor Sample Configuration Files。
...
1. 展開「本地電腦」和「網站」節點。
1. 按一下右鍵要向其中添加收集器的網站，然後選擇「屬性」。
1. 選取「ISAPI篩選」標籤，然後按一下「新增」。
1. 在「篩選器名稱」欄位中，輸入篩選器的顯示名稱。 建議的篩選器名稱為「感測器」。
1. 按一下瀏覽，選擇qlog.dll檔案（位於安裝感測器的目錄中），然後按一下確定。
1. 按一下「確定」以新增篩選。

   新增篩選器後，收集器會立即運作並準備收集資料。 在IIS管理器的「ISAPI篩選器」頁籤的「狀態」列中，應出現向上綠色箭頭。 在流量實際流過篩選器之前，您可能看不到綠色箭頭。 在這種情況下，您需要向Web伺服器提交請求，以確認收集器工作正常。
...
在某些情況下，您可能不想將輸入表單的值附加至顯示在使用者瀏覽器位址列中或可在HTML程式碼本身檢視的URL。 簡單的伺服器端ASP指令碼可讓您將表單欄位名稱及其個別值附加至記錄檔，而不需在使用者的瀏覽器中使用或內嵌至HTML檔案。 若要擷取在網站內特定表單中輸入的實際表單值，必須新增幾行程式碼，才能將表單值附加至記錄請求。
