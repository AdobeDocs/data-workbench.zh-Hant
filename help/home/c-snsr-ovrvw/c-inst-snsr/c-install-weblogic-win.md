---
description: 在Microsoft Windows Server 2000或更高版本下運行的WebLogic Server 6.x或更高版本的感測器安裝和配置的詳細說明。
title: Windows Server 2000或更新版本上的WebLogic Server
...
# Windows Server 2000或更新版本上的WebLogic Server{#weblogic-server-on-windows-server-or-later}

在Microsoft Windows Server 2000或更高版本下運行的WebLogic Server 6.x或更高版本的感測器安裝和配置的詳細說明。
...
1. 在WebLogic伺服器上，建立一個目錄以安裝感測器程式檔案。 請記住，您的磁碟隊列位於此目錄中，因此，請確定您選擇的設備有足夠的空間容納所需大小的隊列。
...
在用於啟動WebLogic的指令碼(例如C:\bea\user_projects\mydomain\startServer.cmd)中，編輯&quot;set JAVA_OPTIONS=&quot;行，將java.library.path定義設定到包含visual_sciences.dll檔案的目錄。
