---
description: Adobe Analytics將更新並增強其資料饋送，為Data Workbench提供動力，以利用Adobe Experience Cloud中的「設定檔與觀眾」。 所有資料工作台使用者都必須準備好在2014年4月21日之前轉換為此新的資料饋送格式。
title: 2014年4月MR的更新
...
# 2014年4月MR的更新{#updates-for-april-mr}

Adobe Analytics將更新並增強其資料饋送，為Data Workbench提供動力，以利用Adobe Experience Cloud中的「設定檔與觀眾」。 所有資料工作台使用者都必須準備好在2014年4月21日之前轉換為此新的資料饋送格式。

「設定檔與對象」提供完整且可完全操作的客戶檢視，以供Adobe Experience Cloud和協力廠商服務使用。 為了開始為Analytics中的新功能建立基礎，新的Experience Cloud ID服務將會在即將推出的4月MR版本中新增至資料饋送，以及其他增強功能與改進。 此新服務適用於任何使用Adobe Experience Cloud的客戶。

此主題中列出此新資料饋送提供的其他增強功能和錯誤修正，以及準備資料工作台組態的指示。

## 新資料饋送的增強功能和錯誤修正 {#section-0013de07da93411d929a1dfd0417f61b}

* 新增Experience Cloud ID服務。
* 新增「目標動作」欄位，以支援Analytics中的Target報告與分析。
* 透過在產品列類型中納入值，改 [!DNL bot_id] 善與「報告與分析」的對齊。
* 減少銷售eVar的大小，而不會遺失資料值的完整性。
* 已解決某些雜湊值會顯示在銷售eVar中的問題。
* 流程中的內部改進可持續提供更佳的穩定性和效能。

## 更新資料工作台設定 {#section-381fcd185af6474bb656501c096c451d}

變更資料工作台將可確保在轉換後，資料饋送的不中斷整合。 在您下次計畫的重新處理更新期間，在設定的記錄處理階段中，目前的解碼器群組。 負責管理資料工作台架構的團隊將能夠執行其他工作，以協助公開並運用新的Experience Cloud訪客識別碼。

**更新資料工作台的任務**

1. 找出目前的解碼器群組。 通常，位於「描述檔管理員」中的「資料集\記錄 [!DNL Instructions.cfg] 處理\解碼」中。
1. 在設定檔中，以滑鼠右鍵按一下目前的解碼器格式，然後選取「複製」。
1. 以滑鼠右鍵按一下並貼上目前解碼器格式下方的新項目。
1. 展開新的解碼器格式，並新增兩個新欄位位置至清單結尾。 如果您尚未準備好在資料集中使用新欄位，或依下列方式定義這些位置，這些位置可能會留空。

   * [!DNL x-mcvisid]
   * [!DNL x-tnt-action]

## 解碼指示 {#section-ba63132ce9604851ad319b6181011820}
